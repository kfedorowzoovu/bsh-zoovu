<template>
  <div :class="[componentStyle.container]" v-if="productRecommendation">
    ...
    <div v-if="iconList.length !== 0" :class="[componentStyle.iconList]">
      <img v-for="icon in iconList" :src="icon" alt="campaign" :key="icon" />
    </div>
  </div>
</template>
...
export default class ProductImageComponent extends Vue {
  ...
  get productImageAriaLabelText(): string {
    return this.$t(
      this.accessibilityContext.accessibility.standardTexts.productImageText,
      {
  ...
  get iconList(): string[] {
    const list = getRecommendationPropertyValue(
      this.productRecommendation,
      this.componentConfiguration.campaignIconsColumn,
    ) as string;
