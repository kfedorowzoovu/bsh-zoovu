<template>
    <button :class="[componentStyle.container]" @click="showRecommendations" >
        <span v-if="showMoreText">{{ $t(componentConfiguration.buttonLabel) }}</span>
...
export default class LoadMoreComponent extends Mixins(ZoovuFacadeMixin) {
    ...
    isWindowInnerWidth = (): boolean => window.innerWidth > this.MIN_DESKTOP_WIDTH;
    ...
    async mounted() {
        ...
        this.isLargeScreen = this.isWindowInnerWidth();
        this.getElement(ClassEnumValues.TARGETED_CLASS_NAME).style.display = ClassEnumValues.CLASS_HIDDEN_STATE;
    }
    ...
    changeInitiallyExpanded() {
        ...
        const element = this.getElement(ClassEnumValues.TARGETED_CLASS_NAME) as HTMLElement;
        const productSortDropdown = this.getElement('[class^="productSortBy"]') as HTMLElement;
    ...
    get monitorClusters(): boolean {
        return this.zoovuFacade.useProduct()?.hasProductsByCluster(2);
    }

    adjustDropDown(): void {
        const getHeight = (selector: string) => (this.getElement(selector) as HTMLElement)?.offsetHeight ?? 0;
        const totalHeight = getHeight(".zv-advisor-cluster-number-0") + getHeight(".zv-advisor-cluster-number-1") - 15;
        const productSortDropdown = this.getElement('[class^="productSortBy"]') as HTMLElement;
    ...
    showRecommendations(): void {
        const element = this.getElement(ClassEnumValues.TARGETED_CLASS_NAME) as HTMLElement;
        const productSortDropdown = this.getElement('[class^="productSortBy"]') as HTMLElement;
        const isHidden = element.style.display === ClassEnumValues.CLASS_HIDDEN_STATE;

        element.style.display = isHidden ? ClassEnumValues.CLASS_VISIBILITY_STATE : ClassEnumValues.CLASS_HIDDEN_STATE;
