<template>
  <div v-if="shouldRenderAnyPrice" :class="[componentStyle.container]">
    <div
      v-if="shouldRenderPriceWithDiscount"
      :class="[componentStyle.priceContainer]"
    >
    ...
    <div
      v-if="
        shouldRenderPriceDescription ||
        isShopCatalogType ||
        shouldRenderFixedPromotion
      "
...
export default class ProductPriceComponent extends Mixins(FormatAsPriceMixin) {
  ...
  async created(): Promise<void> {
    ...
    this.priceVisibilityState = getContextValue(
      this.zoovuFacade,
      'displayProductPrice',
      'boolean',
    ) as boolean;
    ...
    if (this.isEditMode) {
      ...
      return Boolean(
        this.productRecommendation &&
          ...
          parseFloat(
            this.productRecommendation.price.value.toString(),
          ).toString(),
      );
    }
  }
  ...
  get isDiscountPriceLowerThenOriginalPrice(): boolean {
    const discountPriceValue: number = parseFloat(
      this.productRecommendation.price.value.toString(),
    );
    const originalPriceValue: number = parseFloat(
      this.recomendationsPriceValue,
    );

    return (
      !isNaN(discountPriceValue) &&
      !isNaN(originalPriceValue) &&
      discountPriceValue < originalPriceValue
    );
  }
  ...
  get price(): string {
    return this.priceDecorator(
      this.productRecommendation.price.value.toString(),
      this.productRecommendation.price.valueWithCurrency,
    );
  }
  ...
  priceDecorator(value: string, valueWithCurrency?: string): string {
    let price = value;
    if (
      !this.componentConfiguration.hideCurrency &&
      !this.componentConfiguration.showCommaDash
    ) {
      return this.isEditMode || !valueWithCurrency
        ? this.formatValueAsPrice(price)
        : valueWithCurrency ?? '';
    }
    ...
    return this.componentConfiguration.hideCurrency
      ? this.formatAsPriceWithoutCurrency(price)
      : price;
  }
