<template>
  <div
    v-if="shouldDisplayRetailerLogos || showButtonInEditorMode"
...
export default class RetailerLogosView extends Mixins(ZoovuFacadeMixin) {
  ...
  async mounted() {
    ...
    this.isRetailerLogosEnabled = Boolean(
      getContextValue(this.zoovuFacade, 'displayRetailersLogos', 'boolean'),
    );
    this.executeRetailerLogoApiRequest();
  }

  get privacyPolicyText(): string {
    const privacyPolicyUrl: string = this.$t(
      this.componentConfiguration.retailerLogosPrivacyPolicyUrl,
    ).toString();
    let privacyPolicyStandardText: string = this.$t(
      this.componentConfiguration.privacyPolicy,
    ).toString();
  ...
  get endPoint(): string {
    return (
      this.componentConfiguration.apiEndPointUrl ??
      '/ajax/dealer/integration/button/'
    );
  }

  get endPointURL(): string {
    const countryCode =
      this.productRecommendation?.properties[ProductProperty.COUNTRY_CODE]
        ?.value;
