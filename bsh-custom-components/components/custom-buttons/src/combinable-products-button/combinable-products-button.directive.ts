...
export const ProductClickOutLink: Vue.DirectiveOptions = {
  bind(element: HTMLAnchorElement, binding, vNode) {
    ...
    const recommendation: Product = binding.value || vNode?.context?.["recommendation"];
    const customURLProperty: string = recommendation && (vNode?.context?.["customURLPropertyName"] ?? "");
    const recommendationURL: string =
      recommendation && customURLProperty && (vNode?.context?.[customURLProperty] ?? "");
    const regex = /url=[^&]+/;
    ...
    if ((!product && !recommendation) || element.tagName !== "A") {
      if (process.env.NODE_ENV !== "production")
        console.warn(`"v-product-click-out-link" directive should be used only with A tag!`);
      return;
    }
    if (product) {
      ...
      recommendation && !isPdf
        ? element.setAttribute("href", recommendation.getTrackedOfferUrl().replace(regex, newEncondedUrl))
