...
export const ProductClickOutLink: Vue.DirectiveOptions = {
  bind(element: HTMLAnchorElement, binding, vNode) {
    ...
    const recommendation: Product =
      binding.value || vNode?.context?.['recommendation'];
    const customURLProperty: string =
      recommendation && (vNode?.context?.['customURLPropertyName'] ?? '');
    const recommendationURL: string =
      recommendation &&
      customURLProperty &&
      (vNode?.context?.[customURLProperty] ?? '');
    const regex = /url=[^&]+/;
