<template>
  <div
    ...
    @click.stop.prevent
  >
    <div :class="[componentStyle.overlay, { 'transparent-overlay': !hasIframe }]" @mouseup.left.stop="hideModal" />
    <div :class="[componentStyle.modal, { 'zv-info-text-popup': !hasIframe }]">
      <div :class="componentStyle.modalArrow" class="modal-arrow-top-angle" v-if="!hasIframe"></div>
      <button
      ...
      />
      <p :class="[componentStyle.modalText, { 'zv-popup-text': !hasIframe }]" tabindex="0" v-html="infoText" />
    </div>
  </div>
</template>

<script lang="ts">
  import { Component, ComponentConfig, ComponentStyle, Vue, Prop } from "@zoovu/runner-browser-api";
  import { InfoTextModalConfiguration } from "../custom-info-text-trigger.configuration";
  import { infoTextModalStyles, InfoTextModalClasses } from "./info-text-modal.styles";
