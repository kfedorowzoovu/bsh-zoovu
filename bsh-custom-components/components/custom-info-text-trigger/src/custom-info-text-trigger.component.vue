<template>
  <div v-if="infoText" :class="componentStyle.container" class="zv-info-text" :style="iconLocationAdjustment">
    <button
      ...
      @click.stop.prevent
    >
      <div :class="[componentStyle.tooltip, 'zv-answer-tooltip']" v-if="hasIframe">
...
<script lang="ts">
  import { Component, ComponentConfig, ComponentStyle, Prop, Vue } from "@zoovu/runner-browser-api";
  import { CustomInfoTextTriggerConfiguration } from "./custom-info-text-trigger.configuration";
  import { customInfoTextTriggerStyles, CustomInfoTextTriggerClasses } from "./custom-info-text-trigger.styles";
  ...
  export default class CustomInfoTextTriggerComponent extends Vue {
    ...
    get modalPosition(): { left: string; top: string } {
      ...
      const left = `-${this.componentConfiguration.infoTextModal.popupSize.width.value / 2 - iconWidth / 2 - 2}px`;
    ...
    callculatePositionOfInfoTextIcon(): void {
      const parentHeight =
        this.$el.parentElement?.parentElement?.parentElement?.parentElement?.parentElement?.parentElement;
      if (!parentHeight) return;
      const headerImageHeight = this.getElementWithHeight(parentHeight, "header")?.offsetHeight;
      const answerTextContainer = this.getElementWithHeight(parentHeight, ".answer-text-container")?.offsetHeight;
      const infoTextWrapperHeight = this.getElementWithHeight(parentHeight, ".zv-info-text")?.offsetHeight;
      const positionForInfoIcon = headerImageHeight
        ? Number(headerImageHeight) + Number(answerTextContainer / 2 - infoTextWrapperHeight)
    ...
    getElementWithHeight(parentElement: HTMLElement, selector: string): HTMLElement {
      return Array.from(parentElement.querySelectorAll(selector)).find(element => element?.offsetHeight > 0);
    }

    get iconLocationAdjustment(): IconLocationAdjustment {
      if ((this.isDesktopWidth || this.isTabletWidth) && this.componentConfiguration.moveIconToAnswerTextSection) {
        return {
          position: "absolute",
