...
import { createBorderStyle, createPaddingStyles, createFontStyle } from "../../helpers/helpers";

type InfoTextStyleNames = "container" | "icon" | "tooltip" | "tooltipArrow";

export const customInfoTextTriggerStyles: Styles<InfoTextStyleNames, CustomInfoTextTriggerConfiguration> = {
  container: {
    display: "inline-block",
    position: "relative",
  },
  icon: ({ iconBackground, iconHoverBackground, iconColor, width, iconBorder }: CustomInfoTextTriggerConfiguration) => ({
    extend: [
      createBorderStyle(iconBorder),
    ],
  ...
  tooltip: ({ tooltipBackground, tooltipBorder, tooltipPadding, tooltipFont, tooltipWidth }: CustomInfoTextTriggerConfiguration) => ({
    extend: [
  ...
  tooltipArrow: ({ tooltipBackground, tooltipBorder }: CustomInfoTextTriggerConfiguration) => ({
    width: 0,
    height: 0,
...
export type CustomInfoTextTriggerClasses = Classes<keyof typeof customInfoTextTriggerStyles>;
