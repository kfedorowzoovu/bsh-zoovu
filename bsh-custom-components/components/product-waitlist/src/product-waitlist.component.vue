<template>
  <div v-if="shouldRenderWaitListOnProduction" :class="[componentStyle.container]">
    <div
    ...
    >
      ...
      >
        <span class="text">{{ $t(componentConfiguration.subscribeForUnavailableProductText) }}</span>
      </a>
      <aside class="m-popover js-popover" data-t-name="Popover" data-id="">
        ...
        <div class="popover-content">
          ...
          <div :data-actionUrl="markupDataActionUrl" class="m-form js-form">
            <input type="hidden" name="productCode" class="js-input-productCode" :value="markupInputValue" />
            <div class="form-layout-full">
              ...
              <fieldset class="form-row" data-field-count="1">
                ...
                >
                  <span class="text">{{ $t(componentConfiguration.submitText) }}</span>
                </button>
              </fieldset>
            </div>
          </div>
...
<script lang="ts">
  ...
  import { DataColumnName, CatalogVisibilityType, Window } from "../../helpers/types";
  import { ProductWaitlistConfiguration } from "./product-waitlist.configuration";
  import { productWaitlistStyle, productWaitlistStyleType } from "./product-waitlist.style";
  ...
  export default class ProductWaitlist extends Mixins(ZoovuFacadeMixin) {
    ...
    get shouldRenderWaitList(): boolean {
      return (
        ...
        getProductPropertyDefinitionByName(this.productRecommendation, DataColumnName.CatalogVisibilityType)?.value ===
          CatalogVisibilityType.Shop &&
        getProductPropertyDefinitionByName(this.productRecommendation, DataColumnName.Buyable)?.value === false &&
        getProductPropertyDefinitionByName(this.productRecommendation, DataColumnName.PermanentlyNotAvailable)?.value === false
      );
    }
    ...
    await this.zoovuFacade.waitForIdle();
      this.isWaitlistVisibilityState = getContextValue(this.zoovuFacade, "displayWaitlistButton", "boolean") as boolean;
    }
